...
namespace MvcMusicStore.Models
{
    ...
    {
        private static void AddOrUpdate<TEntity>(DbContext context,
                                                 Func<TEntity, object> propertyToMatch,
                                                 IEnumerable<TEntity> entities)
                                           where TEntity : class
        {
            List<TEntity> existingData = context.Set<TEntity>().ToList();
            
            foreach (var item in entities)
            {
                if (!existingData.Any(g => propertyToMatch(g).Equals(propertyToMatch(item))))
                {
                    context.Entry<TEntity>(item).State = EntityState.Added;
                }
            }
            context.SaveChanges();
        }
        public static void Seed(MusicStoreEntities context)
        {
            ...
            AddOrUpdate(context, g => g.Id, genres);
            ...
            AddOrUpdate(context, a => a.Id, artists);
            ...
            AddOrUpdate(context, a => a.Id, albums);
        }
    }
}